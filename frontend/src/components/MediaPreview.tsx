...
const MediaPreview: React.FC<MediaPreviewProps> = ({ item, className = '' }) => {
            ...
            case 'song':
                return (
                    <div className="relative">
                        <motion.div
                            className="w-48 h-48 bg-gradient-to-br from-primary-400 to-purple-500 rounded-full flex items-center justify-center shadow-lg"
                        ...
                        >
                            <svg className="w-24 h-24 text-white" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                    ...
                    <div className="relative w-full max-w-lg select-none">
                        <motion.div animate={{ scale: isHovered ? 1.05 : 1 }} transition={{ duration: 0.3 }}>
                            {isVideoStarted ? (
                                ...
                                <div className="relative w-full h-72 bg-gray-200 flex items-center justify-center cursor-pointer overflow-hidden rounded-lg" onClick={handleVideoPlay}>
                                    {thumbnailUrl ? (
                                        <img
                                            src={thumbnailUrl}
                                            alt={item.title}
                                        ...
                                        <div className="flex flex-col items-center text-gray-500">
                                            <svg className="w-16 h-16 mb-3" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                    ...
                                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center hover:bg-black/40 transition-colors duration-300">
                                        <motion.div className="bg-white/90 rounded-full p-4 hover:bg-white transition-colors duration-200" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                                            <svg className="w-12 h-12 text-gray-800" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" />
                                            </svg>
                                        </motion.div>
                                    </div>
                        ...
                        {isVideoStarted && (
                            <div className="pointer-events-none absolute inset-0 flex items-start justify-end p-3">
                                <div className="bg-black/50 text-white rounded px-3 py-2 text-sm backdrop-blur-sm">
                ...
                return (
                    <div className="relative">
                        <motion.div animate={{ scale: isHovered ? 1.1 : 1, rotateY: isHovered ? 5 : 0 }} transition={{ duration: 0.3 }} className="relative rounded-lg overflow-hidden shadow-lg">
                            <img src={item.mediaUrl} alt={item.title} className="w-full h-72 object-cover" loading="lazy" />
